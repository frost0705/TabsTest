/* --------------------------------------------------------------
    Integral arithmetical calculator.

    http://www.imach.uran.ru/cbignum

    Copyright 2002-2010 by Raul N.Shakirov, IMach of RAS(UB).
    All Rights Reserved.
...
int GetNumberOp (_CIOSTD istream& is, cBigNumber& a)
{
  is >> a >> ws;
  if (a == 0) a.clear();
...
//cerr << ' ' << (const char*) cHexDump (a.loword());
//cerr << ' ' << char(c) /* << '(' << c << ')' */;
//cerr << '\n';
  return c;
}
...
const char szID[] =
    "Big integer calculator. Version 2.0 public of "__DATE__"\n"
    "Copyright 2002-2010 by Dr. Raul N.Shakirov, IMach of RAS(UB)\n"
    ;
const char szHelp[] =
    ...
    "        positive number of length n instead of value n\n"
    "        negative number of length n instead of value -n\n"
    "        number -8,-4,-2,-1,0,1,2,4,8 instead of 0 if option -rand 0\n"
    "        but keep degree of shift and power, unless option -rand all\n"
    "-exp n  append up to n trailing zeroes to random numbers\n"
    "-size   show high word and size of numbers\n"
    "-time   show computing time estimation in Athlon cycles\n"
...
int main (int argc, char** argv)
{
  ...
  {
    pszOutput = "Run concurrent thread for output\n\n";
  ...
  #ifdef  _CBNL_ML
    #define DMUL2 (67 * sizeof (CBNL) * sizeof (CBNL) / 8)
  #else //_CBNL_ML
    #define DMUL2 (46 * sizeof (CBNL) * sizeof (CBNL) / 8)
  #endif//_CBNL_ML
  ...
  do
  {
    ...
    if (ch != EOF && ch != '=')
    {
      op = ch; ch = GetNumberOp (*pin, b);
    }

    if (ch != EOF && ch != '=')
    {
      md = ch; ch = GetNumberOp (*pin, c);
    }
    ...
    if (op != 0)
    {
      ...
      switch (op)
      {
        case '+':   if (pp == op && a == 0) { op = +1; a = b; arb = 1; break; }
                    if (nTime) { nt = (na < nb? na: nb); nt /= DADD; }
                    break;
        case '-':   if (pp == op && a == 0) { op = -1; a = b; arb = 1; break; }
                    if (nTime) { nt = (na < nb? na: nb); nt /= DADD; }
                    break;
    ...
    {
      error |= (isspace (ch) - 1) & 0x8000;     // Skip rest of line
    }

    if (error & 0x0001)
      cerr << "Error: 1st operator is either missed or unknown" << endl;

    if (error & 0x0002)
      cerr << "Error: 2nd operator is either missed or unknown" << endl;
    ...
    if (error & 0x4000)
      cerr << "Error: excessive operators or operands before sign =" << endl;

    if (error & 0x8000)
      cerr << "Error: misplaced operators or operands after sign =" << endl;
    ...
    if (nCheck != 0 && op != 0 && alt != 0)
    {
      ...
      switch (op)
      {
        ...
        case '/': cerr <<"a/b,a%b";     p = a / b; q = a % b;   break;
        case '%': cerr <<"a%b";         p = a % b;              break;
        case '\\':cerr <<"a";           p = a;
                  switch (b.loword()) {
...
static void ShowCycles (cBigNumber& nt, long nKHz)
{
  ...
  cerr << " Athlon cycles";
  if (nKHz)
    cerr << " or " << setw (5) << ((nt + nKHz / 2) / nKHz) << " ms";
}
